# LangGraph 容错专栏文章 实现计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在 langgraph-notes 专栏新增第 5 篇文章 `05-fault-tolerance`，系统讲解 LangGraph 容错机制（超时/重试/错误处理/默认配置/Functional API/优雅关闭），通过 `pnpm build` 验证。

**Architecture:** 单文件 `.mdx` 内容创作。不涉及组件、路由、schema 改动。按 spec 的 11 节结构分批写入：先建文件 + frontmatter（确保 schema 校验通过），再逐章节填充，最后做构建验证与提交。

**Tech Stack:** Astro 5 + MDX + KaTeX + astro-mermaid。frontmatter 必须符合 `src/content.config.ts` 里 `columnArticles` schema（必填 `title` + `createdAt`，用 `order` 排序）。

**Spec:** `docs/superpowers/specs/2026-06-18-langgraph-fault-tolerance-article-design.md`

---

## 写作约定（所有任务通用，遵守 AGENTS.md）

- **frontmatter**：必填 `title` + `createdAt`；含 `subtitle: ''`、`poster`（沿用专栏统一 `photo-1635070041078`）、`tags`、`order: 5`、`draft: false`。专栏文章 schema **没有** `lang`/`category`/`permalink` 字段，别加。
- **数学公式**：用 KaTeX 语法 `$x$` / `$$x$$`，**绝不**用反引号包裹（反引号会原样显示成文本）。
- **Mermaid**：用 ` ```mermaid ` 代码块。节点文字含特殊字符或换行时用**双引号包裹**并用 `<br/>` 换行，例如 `A["① 局部计算<br/>各处理器干活"]`。
- **代码块**：全部 TypeScript（` ```ts `）。不用 ` ```text ` 伪代码块（与用户选择一致）。
- **引用**：交叉引用专栏内文章时用相对路径或文章名，例如「详见《LangGraph Interrupts 详解》」「superstep 概念见《LangGraph 的根基：BSP 与 Pregel》」。

## 构建验证约定（Task 11 集中执行，依据 AGENTS.md）

1. `pnpm build` 必须退出码 0（含 `astro check` + frontmatter schema 校验）
2. 构建日志打印 `[astro-mermaid] Remark transformed mermaid block #N in 05-fault-tolerance`，确认块数 ≥2
3. 产物 `dist/columns/langgraph-notes/05-fault-tolerance/index.html` 存在，且含 `class="katex"`（§4 backoff 公式）
4. 构建日志页面列表里出现 `05-fault-tolerance`

---

### Task 1: 建文件骨架（frontmatter + 一句话定义 + 一句话总结占位）

先把文件建出来，frontmatter 必须通过 schema 校验。这一步用最小心智模型定位全文骨架，方便后续任务往里填。

**Files:**
- Create: `src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx`

- [ ] **Step 1: 写入文件骨架**

完整内容（注意 frontmatter 字段严格符合 columnArticles schema）：
...
Run: `pnpm build`
Expected: 退出码 0；构建日志页面列表里出现 `05-fault-tolerance`。若有 schema 错误，按错误信息修正 frontmatter（典型错误：误加了 `lang`/`permalink`/`category` 字段）。

- [ ] **Step 3: 提交**

```bash
git add src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx
git commit -m "feat(langgraph): 容错专栏文章骨架（第 5 篇）"
```
...
填充 §1。核心：引用官方"three composable mechanisms"，点出容错 = 三段式管线 + 图级默认 + 优雅关闭。

**Files:**
- Modify: `src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx`（替换「一句话定义」节占位）

- [ ] **Step 1: 替换占位，写入完整内容**

把 `<!-- 一句话定义 + 三机制概览，Task 2 填充 -->` 替换为：
...
- [ ] **Step 2: 不必单独构建，进入下一任务（节省构建时间）**

---

### Task 3: 写「心智模型」节（含 mermaid 图）

填充 §2。核心是一张 mermaid 流程图展示"组合顺序"——这是全文最关键的认知锚点。

**Files:**
- Modify: `src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx`（替换「心智模型」节占位）

- [ ] **Step 1: 替换占位，写入完整内容**

把 `<!-- mermaid 流程图 + 组合顺序说明，Task 3 填充 -->` 替换为：
...
注意：mermaid 图里 `HD` 和 `BUBBLE2` 两个结束节点是为了说明 errorHandler 的两种结局，不是死循环。

- [ ] **Step 2: 进入下一任务**

---

### Task 4: 写「Timeouts」节（§3，含三个机制 + heartbeat + NodeTimeoutError）

这是笔记里完全没有、需新学补写的一节，内容最多。分 7 个子小节。

**Files:**
- Modify: `src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx`（在「心智模型」节 `---` 之后插入）

- [ ] **Step 1: 在心智模型节后插入完整 Timeouts 节**

在 Task 3 写入的「心智模型」节结尾的 `---` 之后、`<!-- 后续章节由 Task 4-10 填充 -->` 之前，插入：
...
### Task 5: 写「Retries」节（§4，含修正笔记认知 + retryPolicy 参数 + blocklist）

这一节要**显式修正**笔记里"默认所有异常都重试"的旧认知——官方实际有内置 blocklist。

**Files:**
- Modify: `src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx`

- [ ] **Step 1: 在 Timeouts 节后插入完整 Retries 节**

在 Task 4 插入的「Timeouts」节结尾 `---` 之后插入：
...
### Task 6: 写「executionInfo」节（§5，含字段表 + FAQ）

融入笔记 FAQ：checkpointId/checkpointNs 何时存在、为何 threadId 依赖 checkpointer。

**Files:**
- Modify: `src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx`

- [ ] **Step 1: 在 Retries 节后插入完整 executionInfo 节**

在 Task 5 插入的「Retries」节结尾 `---` 之后插入：
...
### Task 7: 写「Error handling：Saga 补偿」节（§6）

融入笔记的 Saga 部分 + 官方的 Command 路由实现。

**Files:**
- Modify: `src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx`

- [ ] **Step 1: 在 executionInfo 节后插入完整 Error handling 节**

在 Task 6 插入的「executionInfo」节结尾 `---` 之后插入：
...
### Task 8: 写「setNodeDefaults」节（§7）

笔记没有，官方新增。重点是 Precedence / Applicability / Scope 三张表。

**Files:**
- Modify: `src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx`

- [ ] **Step 1: 在 Error handling 节后插入完整 setNodeDefaults 节**

在 Task 7 插入的「Error handling」节结尾 `---` 之后插入：
...
### Task 9: 写「Functional API」节（§8）

笔记没有，官方新增。重点是 task 用 `retry`（不是 `retryPolicy`）这个命名陷阱。

**Files:**
- Modify: `src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx`

- [ ] **Step 1: 在 setNodeDefaults 节后插入完整 Functional API 节**

在 Task 8 插入的「setNodeDefaults」节结尾 `---` 之后插入：
...
### Task 10: 写「Graceful shutdown」节（§9）+「设计规则」+「一句话总结」

含 mermaid 时序图。superstep 概念交叉引用 04-pregel-bsp。

**Files:**
- Modify: `src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx`

- [ ] **Step 1: 在 Functional API 节后插入完整 Graceful shutdown 节**

在 Task 9 插入的「Functional API」节结尾 `---` 之后插入：
...
- [ ] **Step 2: 替换「设计规则」和「一句话总结」占位**

把 `<!-- 后续章节由 Task 4-10 填充 -->` 替换为：
...
### Task 11: 构建验证（依据 AGENTS.md「构建验证约定」）

这是全文写完后的总验证，必须逐项检查产物，不能只看退出码。

- [ ] **Step 1: 运行完整构建**

Run: `pnpm build`
Expected: 退出码 0。若失败，按错误信息修正（常见：frontmatter 字段名错、mermaid 语法错、MDX 语法错）。

- [ ] **Step 2: 验证文章被收录**

在构建日志的页面列表里查找 `05-fault-tolerance`。
Expected: 出现 `/columns/langgraph-notes/05-fault-tolerance/index.html` 或类似条目。

- [ ] **Step 3: 验证 mermaid 真渲染**

在构建日志里查找 `[astro-mermaid]`。
Expected: 至少出现 2 条 `Remark transformed mermaid block #N in ...05-fault-tolerance...`（§2 flowchart + §9 sequenceDiagram）。

- [ ] **Step 4: 验证 KaTeX 真渲染**

Run（在仓库根目录）: `find dist -path "*05-fault-tolerance*" -name "index.html" | head -1 | xargs grep -l "katex"`
Expected: 输出一个 html 文件路径（说明产物含 KaTeX 渲染结果，§4 的 backoff 公式正常渲染）。
若 Windows 下 `find` 不可用，改用: 在 `dist/columns/langgraph-notes/05-fault-tolerance/index.html` 里搜索字符串 `katex`，应能找到。

- [ ] **Step 5: 验证 mermaid 块数与内容**

打开（或读取）`dist/columns/langgraph-notes/05-fault-tolerance/index.html`，确认：
- 包含 §2 的 flowchart（应有 `attempt` / `retry policy` 等文字）
- 包含 §9 的 sequenceDiagram（应有 `requestDrain` / `GraphDrained` 等文字）

- [ ] **Step 6: 提交**

```bash
git add src/content/columns/langgraph-notes/05-fault-tolerance/index.mdx
git commit -m "feat(langgraph): 容错专栏文章完成（第 5 篇）"
```
...
**1. Spec coverage 核对**：spec 的 11 节全部映射到 Task（§1→Task2, §2→Task3, §3→Task4, §4→Task5, §5→Task6, §6→Task7, §7→Task8, §8→Task9, §9+§10+§11→Task10），构建验证→Task11。无遗漏。

**2. Placeholder 扫描**：无 "TBD/TODO"。所有代码块均为完整可粘贴内容。

**3. 命名一致性核对**：
- `retryPolicy`（StateGraph）vs `retry`（task）——Task9 显式提示陷阱 ✅
- `isNodeTimeoutError` ——Task4、Task10 设计规则一致 ✅
- `runtime.heartbeat()` / `runtime.executionInfo` / `runtime.control.drainRequested` ——Task4/6/10 一致 ✅
- `NodeTimeoutError` 字段（type/timedOutAtMs/kind/signals）——Task4 定义，Task10 设计规则引用 ✅
- `Command({ goto })` ——Task7 使用 ✅

**4. 风险点**：mermaid 节点文字含特殊字符（`{`、`？`、`/`）已用双引号包裹 + `<br/>` 换行，符合 AGENTS.md 约定。
