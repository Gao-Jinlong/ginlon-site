...
本文档讲解 DeerFlow 如何通过 FastAPI 的 `app.state` 机制实现进程级单例的依赖注入，以及 `RunContext` 的设计。
...
进程启动
  ↓
FastAPI 调用 lifespan(app)
  ↓
  startup_config = get_app_config()
  ↓
  async with langgraph_runtime(app, startup_config):
    ↓
    app.state.checkpointer = make_checkpointer(config)
    app.state.store = make_store(config)
    app.state.run_manager = RunManager(store=...)
    app.state.run_event_store = make_run_event_store(...)
    app.state.thread_store = make_thread_store(sf, store)
    app.state.run_events_config = ...
    ↓
    yield   ← 进程在这里持续运行
```
...
```
用户请求 → FastAPI 构造 Request
  ↓
  request.app = <FastAPI 实例>     ← 自动持有
  ↓
  get_run_context(request)
    ↓
    get_checkpointer(request)      → request.app.state.checkpointer
    get_store(request)             → request.app.state.store
    ...
    ↓
    return RunContext(checkpointer=..., store=..., ...)
  ↓
  run_agent(ctx=run_context, ...)
```

---

## 2. FastAPI 的 app.state 机制

`app.state` 是 FastAPI 提供的**应用级共享命名空间**：

- **进程级单例**：整个进程只有一个 FastAPI 实例，`app.state` 上的属性对所有请求可见
- **类型无关**：可以挂任意 Python 对象（checkpointer、数据库连接池、Manager 等）
